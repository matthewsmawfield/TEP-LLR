...
C********************************************
	...
	character*9 vers
	character*1 ichr
      character*255 filnami,filnamo,filtmp,filnamr
...
CC	if(filtmp(1:1) .EQ. '/') then
	if(filtmp(1:1) .EQ. 'd') then
	  pathop = filtmp
	else
...
C  GET & OPEN THE NORMALPOINT FILE (if this is not recalc)
	if (modeop .EQ. 0) then
CC	  if(filnamo(1:1) .EQ. '/') then
	  if(filnamo(1:1) .EQ. 'd') then
	    pathop = filnamo
	  else
...
C  GET & OPEN THE RESIDUAL FILE
CC	if (modeop .EQ. 0) then
CC	  if(filnamr(1:1) .EQ. '/') then
	  if(filnamr(1:1) .EQ. 'd') then
	    pathop = filnamr
	  else
	    pathop = patheph(1:leneph)//filnamr
	  endif
	  open(24,file=pathop,iostat=ierr)
CC	endif
c================================================================
			else
...
CC	  type *, "Output mini normal point file:"
	  write( *,* ) "Output mini normal point file:"
	  read(indev,1025) filnamo
	  open(23,file=patheph(1:leneph)//filnamo,iostat=ierr)
	endif
CC	type *, "Output residual file:"
	write( *,* ) "Output residual file:"
	read(indev,1025) filnamr
	open(24,file=patheph(1:leneph)//filnamr,iostat=ierr)
      endif
1025	format(a)
1026	format(a1)
c================================================================
