...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C 10 Dec 01 ?    : add -f switch and name for control file. rlr.
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
c	  process mini normal point file name
CC	  lenb=igetarg(nargs+1,anarg,255)
	  call getarg(nargs+1,anarg)
          lenb= itrimlen(anarg)
	  if(lenb.EQ.-1 .OR. anarg(1:1).EQ.'-') then
	    errmsg=' no mininormal point file name after "-m" option.'
     1							//char(7)
	    call error(2,errmsg,0,nargs)
	  endif
	  nargs=nargs+1
...
c	   the
	...
	function iprdargs(autoprd, ndays, ephname, iterdir, earthvelrecd)
...
C
	...
	character anarg*255, msg*80, d1*1,d2*1,d3*1
	character ctrlfile*14
        character ephname*256
	logical autoprd,earthvelrecd
        integer iterdir
...
c------------------------------------------------------------------
	if(lena .GT. 2) then
	  ...
	  read(d1,5001)idig1
c	  check for 1 - 3 digits:
c..................................................................
	  if(lena .EQ. 3) then
c	    one digit
	    ndays = idig1
	  else if(lena .EQ. 4) then
c	    two digits
	    d2 = anarg(4:4)
...
c	    three digits
	    ...
	    read(d2,5001)idig2
	    d3 = anarg(5:5)
	    if(llt(d3,'0') .OR. lgt(d3,'9')) then
	      msg=' Bad char in command line argument: '//d3
	      call error(2,msg,narg,lena)
	    endif
	    read(d3,5001)idig3
	    ndays = 100*idig1 + 10*idig2 + idig3
	  endif
c..................................................................
	endif
c------------------------------------------------------------------
...
CC	    lena=igetarg(narg,anarg,255)
	    call getarg(narg,anarg)
            lena= itrimlen(anarg)
	    if(anarg(1:2).EQ.'-a' .OR. anarg(1:2).EQ.'-f'
     x		.OR. anarg(1:2).EQ.'-e') then
	      msg=' No file name follows "-p" on command line.'
	      call error(2,msg,narg,numargs)
	    endif
	    filpol=anarg
...
CC Prediction control file name is overridden...
	  elseif (anarg(1:2) .EQ. '-f') then
	    ...
	    call getarg(narg,anarg)
            lena= itrimlen(anarg)
	    if(anarg(1:2).EQ.'-a' .OR. anarg(1:2).EQ.'-p'
     x		.OR. anarg(1:2).EQ.'-e') then
	      msg=' No file name follows "-f" on command line.'
	      call error(2,msg,narg,numargs)
	    endif
     ...
     X		//char(7)
	    ...
	    FNAME(6)=anarg
c + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + +
CC Prediction ephemeris file name is set...
	  elseif (anarg(1:2) .EQ. '-e') then
	    narg=narg+1
	    if(narg.GT.numargs) then
	      msg=' No file name follows "-e" on command line.'
	      call error(2,msg,narg,numargs)
	    endif
	    call getarg(narg,anarg)
            lena= itrimlen(anarg)
	    if(anarg(1:2).EQ.'-a' .OR. anarg(1:2).EQ.'-p'
     x		.OR. anarg(1:2).EQ.'-f') then
	      msg=' No file name follows "-e" on command line.'
	      call error(2,msg,narg,numargs)
	    endif
	    if(lena.GT.256) then
	      msg=' ephemeris file name too long.'
     X		//char(7)
	      call error(2,msg,2,nargs)
	    endif
	    ephname=anarg
c + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + +
CC Perform predictions backwards in time
	  elseif (anarg(1:2) .EQ. '-r') then
	    iterdir= -1
c + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + +
CC Prediction ephemeris file name is set...
	  elseif (anarg(1:3) .EQ. '-31') then
	    earthvelrecd= .true.
c + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + +
	  else
