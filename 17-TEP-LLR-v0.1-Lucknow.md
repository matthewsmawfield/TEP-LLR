...
where $r_i$ is the residual, $D_i$ is the elongation, and $\sigma$ is the noise standard deviation. Uniform priors were adopted: $\eta \in [-10^{-3}, +10^{-3}]$ and $\sigma \in [0.05, 0.15]$ m. The sampler used 32 walkers with 3,000 steps each (1,000 burn-in, 64,000 post-burnin samples). Convergence was verified using the Gelman-Rubin statistic ($\hat{R} < 1.01$ required). The Savage-Dickey Bayes Factor was computed by comparing the posterior density at $\eta = 0$ to the prior density at $\eta = 0$:
...
The detected signal is frequency-specific: significant power appears at the synodic fundamental (4.92$\sigma$ full-sample correlation, 4.93$\sigma$ OLS) with no detectable signal at 55 tested non-synodic frequencies spanning 0.4×–3.2× (maximum ≈ 0.00σ). The 1.23× control factor — a theoretically motivated non-physical frequency — shows ≈ 0.00σ significance, consistent with pure noise. This frequency pattern is expected for a physical effect phase-locked to Earth-Moon-Sun geometry, whereas broadband systematic artifacts would typically generate spurious power across multiple frequencies.
