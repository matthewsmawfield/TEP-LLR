...
def run_differential_analysis(df, verbose=False):
    print_status("═══ Starting Step 010: Ephemeris-Independent Analysis...", "TITLE")
    ...
    print_status(f"    Output file: results/outputs/step_010_ephemeris_independent_analysis.json", "INFO")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Step 010: Ephemeris Independent Analysis")
    args = parser.parse_args()

    log_dir = PROJECT_ROOT / "logs"
    logger = TEPLogger("step_010", str(
        log_dir / "step_010_ephemeris_independent_analysis.log"))
    ...
    logger.save_step_results(results, PROJECT_ROOT,
                             "step_010_ephemeris_independent_analysis")
