...
def generate_systematic_error_budget(df, systematics, verbose=False, logger=None):
    ...
    # CRITICAL FIX: subtract the FULL model (slope + intercept), not just the slope.
    # Leaving the intercept in detrended residuals introduces a constant offset
    # into the systematic error budget.
    detrended = residuals - (coeffs_tep[0] * cos_elong + coeffs_tep[1])  # m
