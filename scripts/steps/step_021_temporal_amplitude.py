...
def main():
    ...
    logger = TEPLogger("step_021", str(log_dir / "step_021_temporal_amplitude.log"))
    set_step_logger(logger)

    print_status("═══ Starting Step 021: Temporal Amplitude Evolution Analysis...", "TITLE")
    ...
    print_status(f"    Output file: results/outputs/step_021_temporal_amplitude.json", "INFO")
    print_status(f"    Window size: 5 years", "INFO")
    print_status(f"    Step size: 2 years", "INFO")

    # Save results
    project_root = Path(__file__).parent.parent.parent
    output_path = project_root / 'results' / 'outputs' / 'step_021_temporal_amplitude.json'
