...
def test_balanced_subsample(residuals, cos_elong, stations, n_per_station=None, seed=TEP_CONFIG.get("RANDOM_SEED", 42)):
    ...
    for s in unique_stations:
        ...
        if len(idx_s) >= n_per_station:
            chosen = rng.choice(idx_s, size=n_per_station, replace=False)
...
def main():
    ...
    for _ in range(n_rand):
        pole_rand = rng.permutation(pole_range)
    ...
    results = {
        ...
        "conclusion": "Pole tide contributes modestly to the cosD-only signal (0.4σ reduction), "
                      "and the signal remains negative after pole-tide correction. "
                      "The equal-N balanced subsample is underpowered and near zero both before "
                      "and after EOP correction, so this step supports 'EOP is not the dominant "
                      "driver' rather than diagnosing a full root cause for station imbalance."
    }
