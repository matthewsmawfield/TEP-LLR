...
def main():
    ...
    logger = TEPLogger("step_022", str(log_dir / "step_022_ipw_validation.log"))
    ...
    logger.info("Step 022: IPW Criteria Monte Carlo Validation")
    ...
    results = validate_ipw_criteria(
        eta_true, station_fractions, n_mc=500, seed=42)

    # Add metadata
    results['step_id'] = 'step_022'
    results['status'] = 'PASS'
    ...
    print_status(f"    Output file: results/outputs/step_022_ipw_validation.json", "INFO")
    ...
    output_path = Path(__file__).parent.parent.parent / \
        'results' / 'outputs' / 'step_022_ipw_validation.json'
