...
def per_station_power_analysis(df: pd.DataFrame, eta_measured: float,
    ...
    print_status("═══ Starting Step 031: Station Power & Grasse Dominance Assessment...", "TITLE")
...
def run_station_power_analysis(verbose: bool = False) -> dict:
    ...
    print_status(f"    Output file: results/outputs/step_031_station_power_analysis.json", "INFO")
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Step 031: Station Power Analysis')
    parser.add_argument('--verbose', '-v', action='store_true')
    args = parser.parse_args()

    log_dir = PROJECT_ROOT / 'logs'
    log_dir.mkdir(parents=True, exist_ok=True)
    logger = TEPLogger('step_031', str(log_dir / 'step_031_station_power_analysis.log'))
    set_step_logger(logger)

    print_status('Starting Step 031: Station Power & Grasse Dominance Assessment...', 'TITLE')
    results = run_station_power_analysis(verbose=args.verbose)

    output_path = PROJECT_ROOT / 'results' / 'outputs' / 'step_031_station_power_analysis.json'
