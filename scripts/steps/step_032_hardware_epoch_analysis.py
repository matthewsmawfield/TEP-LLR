...
def run_hardware_epoch_analysis(verbose: bool = False) -> dict:
    print_status("═══ Starting Step 032: Hardware Epoch Consistency Analysis...", "TITLE")
    ...
    print_status(f"    Output file: results/outputs/step_032_hardware_epoch_analysis.json", "INFO")
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Step 032: Hardware Epoch Consistency')
    parser.add_argument('--verbose', '-v', action='store_true')
    args = parser.parse_args()

    log_dir = PROJECT_ROOT / 'logs'
    log_dir.mkdir(parents=True, exist_ok=True)
    logger = TEPLogger('step_032', str(log_dir / 'step_032_hardware_epoch_analysis.log'))
    set_step_logger(logger)

    print_status('Starting Step 032: Hardware Epoch Consistency Analysis...', 'TITLE')
    results = run_hardware_epoch_analysis(verbose=args.verbose)

    output_path = PROJECT_ROOT / 'results' / 'outputs' / 'step_032_hardware_epoch_analysis.json'
