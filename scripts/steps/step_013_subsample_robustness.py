...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Step 013: Subsample Robustness")
    args = parser.parse_args()

    log_dir = PROJECT_ROOT / "logs"
    logger = TEPLogger("step_013", str(
        log_dir / "step_013_subsample_robustness.log"))
    ...
    results = {
        ...
        "status": "PASS" if summary["robust"] else "WARNING"
    }

    logger.save_step_results(results, PROJECT_ROOT,
                             "step_013_subsample_robustness")
    logger.save_step_results(
        dominance_report, PROJECT_ROOT, "step_013_station_dominance")
