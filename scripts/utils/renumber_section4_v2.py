...
Approach:
1. Exact replacements for headers (removes step numbers from headers).
2. Exact replacements for specific text patterns (ranges, table cells, captions, intro).
3. Regex replacements for Section/Sections/§ prefixed numbers.
   Scientific notation ($4.18 x 10^-4$) is unaffected because it lacks the prefix.
"""
...
# =============================================================================
# 2. Exact text replacements for known ranges / table cells / captions / body refs
# =============================================================================
EXACT_TEXT_REPLACEMENTS = [
    ...
    # Range references not caught by Section/Sections/§ regex
    ("§4.13.1–4.13.4",
     "§4.19.1–4.19.4"),
    ("Sections 4.18.1–4.18.2",
     "Sections 4.24.1–4.24.2"),
    ("Sections 4.5 and 4.18.5.",
     "Sections 4.10 and 4.24.5."),
    ("Sections 4.8–4.25",
     "Sections 4.14–4.31"),
]

# =============================================================================
# 3. Regex mapping for Section / Sections / § prefixed numbers
# =============================================================================
SECTION_MAP = {
    # sub-sub-sections (process longer first in regex loop)
    "4.18.6": "4.24.6",
    "4.18.5": "4.24.5",
    "4.18.4": "4.24.4",
    "4.18.3": "4.24.3",
    ...
    # sub-sections
    "4.29": "4.6",
    "4.28": "4.13",   # Linearized Post-Fit η Extraction (Step 056)
    "4.27": "4.1",    # Multiple-Testing Consolidation (Step 042)
    "4.26": "4.3",
    "4.25": "4.31",
    "4.24": "4.30",
    "4.23": "4.29",
...
SECTION_NUMBER_RE = re.compile(r'(\d+\.(?:\d+\.)?\d+)')


def replace_section_number(match):
    old = match.group(1)
    return SECTION_MAP.get(old, old)


def apply_section_regex(text):
    """Replace numbers after Section / Sections / §."""
    # Section prefix (8 chars)
    text = re.sub(r'(?<=Section )' + SECTION_NUMBER_RE.pattern, replace_section_number, text)
    # Sections prefix (9 chars)
    text = re.sub(r'(?<=Sections )' + SECTION_NUMBER_RE.pattern, replace_section_number, text)
    # § prefix (1 char)
    text = re.sub(r'(?<=§)' + SECTION_NUMBER_RE.pattern, replace_section_number, text)
    return text
...
def main():
    ...
    for fpath in html_files:
        ...
        # --- Regex-based Section/Sections/§ replacements ---
        text = apply_section_regex(text)
