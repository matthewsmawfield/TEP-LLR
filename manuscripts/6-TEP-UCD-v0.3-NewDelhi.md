...
orders of magnitude in density yields $S \propto \rho^{0.334}$, consistent
with the $\rho^{1/3}$ exponent. This is a consistency check, not
independent evidence: $S \equiv R_T/R_{\rm phys}$ with $R_T \propto
M^{1/3}$ and $R_{\rm phys} \propto (M/\rho)^{1/3}$ implies $S \propto
\rho^{1/3}$ algebraically (see Box 4.1). The hierarchy confirms internal
consistency of the screening framework. At nuclear densities (binary pulsars: $\rho \sim 10^{14}$
...
crit} \approx 6.8$ s, at which the light-cylinder radius equals the
saturation radius. The magnetar 1E 2259+586 ($P = 6.98$ s) exhibits
anti-glitch behavior, matching this threshold within 4%. This is a
single-object consistency check ($N=1$), not an independent validation;
population-level predictions are the testable content (Section 6.3).
...
orders of magnitude in mass (proton to galaxy cluster). A formal
dependency audit (Section 6) identifies two independent constraints
(GNSS calibration and SPARC scaling) with three additional consistency
checks (screening hierarchy, magnetar $P_{\rm crit}$, Milky Way
$R_{\rm trans}$). This externally calibrated value enables tightly
...
- **Milky Way:** Gaia DR3 analyses report evidence consistent with a Keplerian-like decline at $R \approx 19$ kpc; the SPARC-calibrated prediction shows tension (factor of ~2.4–6, Section 8) (Gaia Collaboration 2023; Jiao et al. 2023).
...
*Note: The dependency audit (Section 6) identifies two independent constraints (GNSS, SPARC) and three consistency checks (screening, magnetar, Milky Way). The convergence across 40 orders of magnitude in mass motivates $\rho_T$ as a candidate saturation scale, conditional on independent replication of the GNSS calibration.*

## Paper Structure

*Note on Figures:* All figures referenced in this manuscript are generated by the reproducible analysis pipeline (Steps 001–041) and saved to `results/figures/`. These include rotation curve plots, screening hierarchies, and diagnostic visualizations derived from actual data. Run the pipeline to generate publication-quality figures.
...
the Milky Way Keplerian transition (factor ~2.4–6 tension; Section 8), these results
...
Using the SPARC-calibrated normalization ($k \approx 7.9 \times 10^{-4}$ kpc/$M_\odot^{1/3}$) and the Milky Way's total baryonic mass estimated at $M_{\text{bar}} \approx 6 \times 10^{10} M_\odot$ (Bland-Hawthorn & Gerhard 2016), the predicted baryonic transition radius is:

$ R_{\text{trans}} = k \cdot M_{\text{bar}}^{1/3} \approx 7.9 \times 10^{-4} \cdot (6 \times 10^{10})^{1/3} \text{ kpc} \approx 3.1 \text{ kpc} $

This is the radius at which the baryonic mass alone would predict the transition, before accounting for the phantom mass contribution. The observed Keplerian decline at $R \approx 19$ kpc (Jiao et al. 2023) occurs at a substantially larger radius, indicating that the total dynamical mass governing the outer rotation curve is $M_{\text{dyn}} \sim 10^{12} M_\odot$. Applying the $M^{1/3}$ scaling to this total mass yields $R \approx 7.9$ kpc — still a factor of ~2.4 below the observed 19 kpc transition. This discrepancy may reflect the simplified spherical saturation geometry assumed here, the contribution of the Milky Way's complex baryonic morphology, or systematic uncertainties in the outer-disk rotation curve. The Milky Way comparison is retained as a scale-consistency check but the tension is noted.

## Gaia DR3 Observations

Gaia Data Release 3 (Gaia Collaboration 2023) provides precise proper motions and radial velocities for millions of stars, enabling construction of the Milky Way rotation curve to unprecedented distances. Recent analyses (e.g., Jiao et al. 2023) report evidence consistent with a Keplerian-like decline beginning near $R \approx 19$ kpc. As noted above, this is larger than the SPARC-calibrated prediction; the comparison is treated as a scale-consistency check with acknowledged tension. The inferred outer-disk behavior is sensitive to tracer selection, distance systematics, and modeling of non-circular motions.
...
The error budget is quantified for the SPARC-calibrated prediction.
...
| Total Predicted $R$ (baryonic) | 3.1 kpc | $\pm 0.4$ kpc (11.5%) | - |
| Total Predicted $R$ (dynamical $M \sim 10^{12}$) | 7.9 kpc | $\pm 1.0$ kpc | - |
| **Observed Radius** | 19.0 kpc | $\pm 2.0$ kpc (Gaia) | - |

The SPARC-calibrated prediction ($3.1 \pm 0.4$ kpc for baryonic mass; $7.9 \pm 1.0$ kpc for total dynamical mass) lies below the reported transition scale ($19 \pm 2$ kpc). The discrepancy is approximately 5$\sigma$ relative to the dynamical-mass prediction. This tension may indicate that the simplified spherical saturation geometry underestimates the transition radius in disk-dominated systems, or that the outer-disk rotation curve inferences carry systematic uncertainties not captured by the formal errors. The Milky Way comparison is retained as a scale-consistency check; resolution of the tension requires improved outer-disk kinematic data and non-spherical saturation modeling.

## Comparison with Dark Matter Halo Models

- **NFW Halo (CDM):** The transition to Keplerian decline is soft and depends on two free parameters: the virial mass $M_{\text{vir}}$ and concentration $c$. An NFW profile can be *fitted* to the decline, but does not *predict* the transition location a priori.

- **TEP Soliton:** Predicts the transition radius from the mass using the universal scaling law with no Milky Way-specific tuning. The current prediction underestimates the observed transition by a factor of ~2.4–6 (depending on mass choice), indicating that the simple spherical model requires refinement for disk galaxies.

The tension between the SPARC-calibrated prediction and the Milky Way's reported Keplerian transition is acknowledged. This does not falsify the $M^{1/3}$ scaling — which is independently supported by the SPARC ensemble — but indicates that the normalization or geometry requires system-specific adjustment for the Milky Way's complex morphology.
...
The critical spin period of magnetars ($P_{\text{crit}} \approx 6.8$ s; single-object consistency check, $N=1$).

The dark matter onset radius in spiral galaxies ($R_{\text{DM}} \propto
M^{1/3}$; independent constraint from SPARC ensemble).

The screening hierarchy in binary pulsars vs. galaxies ($S \propto
\rho^{1/3}$; algebraic consistency check).

The Keplerian transition radius of the Milky Way shows tension with the SPARC-calibrated prediction (factor of ~2.4–6; Section 8), indicating that the simplified spherical saturation model requires refinement for disk galaxies.

A formal dependency audit (Section 6) identifies two independent constraints (GNSS calibration and SPARC scaling) with three additional consistency checks. The convergence of these constraints suggests that $\rho_T$ is
not merely a fitting parameter for a specific system, but a candidate
universal parameter of the effective description of the dark sector, conditional on independent replication of the GNSS calibration.
...
- **Cross-scale agreement:** Five tests spanning 40 orders of magnitude; dependency audit (Section 6) identifies two independent constraints and three consistency checks.
...
phenomena across 40 orders of magnitude in mass (dependency audit: Section 6).
