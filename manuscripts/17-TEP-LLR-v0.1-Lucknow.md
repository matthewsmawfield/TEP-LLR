...
The quantitative expectation for the Nordtvedt parameter is framed in the TEP program as an Observable Response Coefficient: a domain-specific macroscopic response rather than a universal bare scalar coupling. Preliminary results from related work report $\kappa_{\rm Cep} = (1.05 \pm 0.43) \times 10^6$ mag for Cepheid period-luminosity anomalies (Paper 11) and $\kappa_{\rm MSP} \sim 10^6$–$10^7$ for pulsar spin-down excess (Paper 10). Those coefficients are not the microscopic conformal coupling $\beta$ or scalar-tensor coupling $\alpha_0$; they absorb environmental activation and transfer functions. For LLR, the phenomenological TEP expectation is therefore not a parameter-free prediction, but a sign and order-of-magnitude expectation: a negative Solar-System response in the $10^{-4}$ regime, smaller than the unscreened galactic responses because the Earth-Moon system is more strongly screened.
...
Two theoretically distinct expectations are evaluated. The *compactness-squared* expression above uses only the bare microscopic coupling $\alpha_0$ and the surface compactness differential. With the Cassini-bound $\alpha_0 \lesssim 3\times10^{-3}$ and $\Delta(\Phi^2) \approx 5\times10^{-19}$, this gives $\eta \approx 4\times10^{-24}$ — essentially zero. This baseline is not a physical prediction for LLR; it is a null consistency check showing that the measured residual-channel $\eta$ cannot be interpreted as a bare microscopic scalar-tensor coupling alone. In the TEP program, $\eta$ is treated as an emergent observable response coefficient for the LLR channel.

The *phenomenological* TEP expectation is obtained from the volumetric suppression model, which integrates interior density profiles rather than surface compactness:
...
The measured $\eta$ is also larger than the standard scalar-tensor upper-bound estimate $\eta \approx 4\alpha_0^2 \approx 3.6\times10^{-5}$ (with $\alpha_0 \lesssim 3\times10^{-3}$ from Cassini), reinforcing that the LLR coefficient should not be read as a bare scalar-tensor parameter. The measured $\eta$ is much smaller than preliminary $\kappa_{\rm MSP} \sim 10^6$–$10^7$ and $\kappa_{\rm Cep} \sim 10^6$ from related work in the same framework (Papers 10 and 11), which is consistent with the screening mechanism: LLR is in a more screened regime (Solar System) compared to globular clusters and galactic disks, so the effective response should be smaller. No separate $\kappa_{\rm LL}$ parameter is required; $\eta$ itself is the observable response coefficient for LLR. The interpretation hierarchy is therefore: (i) bare compactness-squared null check, (ii) volumetric phenomenological order-of-magnitude band, (iii) measured LLR response coefficient $\eta$.
...
The cleaned DE430 dataset exhibits complex behavior that requires careful interpretation. The full dataset shows no significant correlation with $\cos(D)$, while a small number of phase-clustered gross outliers can dominate the correlation statistic. The primary detection therefore relies on the INPOP19a ephemeris (35.5-year baseline) with precision-weighted full-systematic regression $\eta = -3.91 \times 10^{-4} \pm 5.63 \times 10^{-5}$ at $6.94\sigma$ (all $N = 25{,}445$ observations retained). Because only $G=5$ stations contribute, the co-primary uncertainty companion is a wild cluster bootstrap with Webb 6-point weights on the unweighted full-systematic OLS design, giving a $95\%$ percentile interval $[-5.37, -2.73] \times 10^{-4}$ ($5.56\sigma$; wild cluster bootstrap), supporting the inference under Webb weights. A station-era block bootstrap (10 blocks, 5{,}000 draws) yields only $1.40\sigma$ ($p = 0.14$); this weakness is expected because with only $\sim$10 highly unequal blocks the bootstrap standard error conflates structural between-block heterogeneity (phase truncation, noise levels, observational design) with sampling variation. The Webb wild cluster bootstrap remains the appropriate small-$G$ inference tool. The unweighted full-systematic OLS on the same sample, reported as a sensitivity upper bound, gives $\eta = -4.06 \times 10^{-4} \pm 6.58 \times 10^{-5}$ at $6.17\sigma$ ($6.52\sigma$ cluster-robust for comparison). Cook's-Distance excision on the full-systematic design ($N = 23{,}837$) is retained only as a secondary leverage diagnostic ($7.82\sigma$ / $8.65\sigma$ cluster-robust). A detailed analysis of the DE430 outlier behavior and statistical validation is presented in Section 4.33.
...
Based on the differential gravitational compactness between Earth ($\Phi_{\oplus}/c^2 \approx 7 \times 10^{-10}$) and Moon ($\Phi_{\rm Moon}/c^2 \approx 3 \times 10^{-11}$), together with the volumetric suppression model in Section 2, the TEP framework motivates a negative residual-channel Nordtvedt response at the order-of-magnitude level $|\eta| \sim 10^{-4}$. The precise value remains model-dependent rather than parameter-free. Using $\delta r = 13\eta\cos(D)$, the measured $\eta$ amplitudes correspond to range modulations below one centimetre, smaller than the 9.5 cm RMS noise per observation, but recoverable through phase-coherent averaging over the full LLR archive.

### 3.3 Statistical Methods

#### 3.3.1 Overview of Analysis Pipeline

The analysis is organized around a fixed estimator hierarchy: a precision-weighted full-systematic residual regression is the headline estimand; an unweighted full-systematic OLS fit and Cook's-Distance excision bracket sensitivity; a wild cluster bootstrap provides the small-$G$ uncertainty companion; and the remaining diagnostics test station leverage, ephemeris absorption, spectral specificity, and known systematics. For reproducibility, these analyses are implemented as a seven-family pipeline of 82 analysis modules (77 sequentially numbered scripts, 000–076, with five suffix variants):

| Family | Step modules | Purpose | Key Analyses |
| --- | --- | --- | --- |
...
| E | 033–066 (incl. 046b, 062×2, 063×2, 064×2) | Advanced Defensibility & Cross-Validation | Quantitative η prediction, dust sensitivity, station balance, clean subset, CMB robustness suite, Haleakala fluctuation, Grasse sufficiency, Gaussian process extraction, systematic sensitivity (known-systematic MC null tests; adversarial PCA/GP nuisance search; era×station×lunation grid; blind 20% year hold-out), orbital SRP bounds, atmospheric seeing, outlier sensitivity, false-positive simulation, high-dimensional absorption test, sideband survival |
| F | 014–015, 040–043, 055–056, 062–066-PI | Results Consolidation & Spectral Validation | Inter-station meta-analysis, unified results table, multiple-testing correction, ephemeris absorption simulation, sideband survival, high-dimensional absorption test, false-positive simulation, outlier sensitivity, prediction-interval calibration |
| G | 067–076 | Advanced Estimator Corrections | Cluster-robust AR(1) combined standard errors, weighted robust M-estimator, rolling η(t) environmental correlation, DE430 full environmental model, stratified equal-N test, LOSO meta-analysis, Laplace Bayes factor, wild cluster bootstrap, generic PPN test, directional sky scan |

This multi-layered approach separates the primary estimand from robustness tests. The diagnostics do not create additional discovery claims; they ask whether the same synodic coefficient survives plausible statistical, instrumental, and ephemeris alternatives.

Families are conceptual categories and not mutually exclusive; some step modules serve multiple validation objectives (e.g., 014–015 and 040–043 appear in both their primary family and the consolidation family). The pipeline comprises 82 modules in 77 sequentially numbered scripts (000–076), with five suffix variants (006b, 046b, two each at 062, 063, and 064). The core statistical analysis (steps 000–003) provides the primary detection, while steps 004–076 perform extended systematic checks, robustness validation, and theoretical consistency tests.

#### 3.3.2 Pearson Correlation Analysis

As a baseline diagnostic, the analysis computes the Pearson correlation coefficient between the O-C residuals and $\cos(D)$, where $D$ is the Moon-Sun elongation. A significant negative correlation is the simplest expression of the predicted synodic footprint. It is reported for the full dataset and separately for each station, but it is not the headline uncertainty model; that role belongs to the full-systematic regression hierarchy described below.
...
where $r$ is the residual, $b$ is an intercept, and $\epsilon$ represents noise.  An intercept is included in all primary regressions even though INPOP19a residuals are mean-subtracted by construction (mean = 0.0007 m $\approx$ 0).  This guards against any residual station-epoch-dependent offset that would otherwise bias the $\cos(D)$ amplitude.  The fitted coefficient of $\cos(D)$ yields an estimate of $\eta$. The headline model extends this baseline with annual, monthly, thermal, and station-specific nuisance structure, inverse station-variance weighting, and a small-cluster bootstrap uncertainty companion.
...
Four systematic analysis families directly test the artifact hypothesis. Systematic control analysis asks whether the signal persists after controlling for temporal trends, seasonal cycles, and station drifts. Noise injection and signal recovery quantify whether the pipeline recovers injected synodic signals and how much added noise is required to erase them. Subsample robustness implements replication, station jackknife, and IPW station-balance tests. Station decomposition decomposes the pooled fit into station-specific contributions. Detailed results are presented in Section 4.
...
A synodic-only Bayesian layer (emcee; Foreman-Mackey et al. 2013) uses heteroskedastic Gaussian likelihood with archive $\sigma_i$ and uniform priors $\eta \in [-10^{-2}, +10^{-2}]$, $b \in [-0.1, +0.1]$ m. The MCMC analysis runs separate chains for four documented $\eta$ priors (Table 4.8.1) and evaluates the Savage–Dickey ratio at $\eta=0$ with multiple KDE bandwidths. This layer is secondary to the cluster-robust frequentist headline and the linearized integrator extraction.
...
**Station Decomposition.** The pooled regression is decomposed into station-specific $\eta$ contributions to verify that the global negative sign is not driven by a single observatory. Detailed results are presented in Section 4.
...
- Statistical power limitation: With only 737 observations and RMS = 13.8 cm, Haleakala has expected SNR = $0.81\sigma$ at the global $|\eta| \approx 4.16 \times 10^{-4}$, below the $3.0\sigma$ powered-detection threshold
...
Crucially, excluding Haleakala does not weaken the detection. The clean-subset analysis restricted to Grasse 2010+ and APO -- which drops Haleakala, Matera, and McDonald2 entirely -- yields η = -3.36 × 10⁻⁴ ± 4.63 × 10⁻⁵ (7.25σ cluster-robust), confirming the signal is detectable in the highest-quality data alone. The common-η mixed model with station-specific systematics gives *F*(4, 25,410) = 1.19, *p* = 0.31, showing no evidence for station-specific Nordtvedt parameters. The multi-station meta-analysis therefore confirms that the global detection is robust and not driven by any single station.
...
A correlation analysis between the LLR O-C residuals and the predicted TEP Nordtvedt signal modulation $\cos(D)$, where $D$ is the Moon-Sun elongation angle. Section 4.0 states the independent evidence spine; the subsections that follow develop pooling, regression, station-specific systematic testing, and cross-ephemeris validation; later sections treat systematics, spectral specificity, environmental modulation, orthogonality, and robustness testing without reopening the headline synodic estimand. The table below gives the referee path through §4.
...
| 2 | 4.7 | Multiplicity control |
| 3 | 4.12 | Cross-station pooling; leave-one-station-out meta (Figure 5) |
| 4 | 4.14 | Station-balance power stress |
| 5 | 4.15–4.22 | Station-specific systematic testing; core regression and per-station detail |
| 6 | 4.23 | Hardware epochs |
| 7 | 4.27, 4.28 | DE430 comparison; linearized post-fit $\eta$ extraction |
| 8 | 4.20.5, 4.29–4.34 (incl. 4.30.1) | Uncertainty calibration; systematics, CV / nuisance transport, clean subset, orthogonality simulations |
| 9 | 4.7.* | Conditional CMB / heliocentric joint layer |
| 10 | 4.35 | Generic PPN falsification ($\beta$, $\gamma$ joint contour) |
| 11 | 4.36 | Full sky-scan with look-elsewhere correction |
...
### 4.14 Station-Balanced TEP Stress Test
...
### 4.15 Testing for Station-Specific Artifacts
...
### 4.16 Gaussian Process Non-Parametric Extraction
...
### 4.17 Systematic Amplitude Sensitivity
...
#### 4.17.1 Adversarial nuisance search (beyond the known list)
...
**2D Gaussian process on elongation $\times$ time** (455 valid bin means, GP-style gridding): a non-parametric surface partially overlaps the synodic phase sampling and absorbs $\approx 83\%$ of the $\cos(D)$-only amplitude after subtraction ($\eta = +5.3 \times 10^{-5}$, $p = 0.36$). This documents expected collinearity between elongation-phase structure and $\cos(D)$ on a non-uniform sampling manifold.

Three diagnostic tests adjudicate between genuine absorption and overfitting. (i) A cross-validated GP trained on 70% of the bins and extrapolated to the held-out 30% yields an out-of-sample RMSE 42% higher than the in-sample RMSE (overfit ratio $= 1.42$); the cosD coefficient on residuals from this CV-GP is $\eta = +2.45 \times 10^{-5}$ ($0.42\sigma$), indistinguishable from zero. (ii) A time-only GP (no elongation dimension) leaves cosD intact at $\eta = -3.26 \times 10^{-4}$ ($5.50\sigma$), showing that the elongation dimension is where the GP overlaps the synodic signal. (iii) AIC on the full data prefers the GP-only model ($\Delta\mathrm{AIC} = -1{,}450$ relative to cosD-only), but this preference reflects the GP's greater flexibility on the in-sample manifold, not predictive superiority: the CV-GP does not generalise the synodic structure, and the time-only GP leaves it intact. The balance of evidence — failed generalisation on held-out data and preservation under time-only nuisance — supports interpreting the 2D GP absorption as overfitting of a flexible surface to the sparse elongation$\times$time sampling, not as a physical systematic that replaces the synodic component.

#### 4.17.2 Era $\times$ station $\times$ lunation interaction grid
...
### 4.23 Kinematic Signal Extraction and Robust Regression

The data are fit to the model $R = A \cos(D) + \epsilon$, where $R$ is the residual, $A$ is the amplitude, and $\epsilon$ is noise. Precision astronomical telemetry—particularly from early-epoch ground stations (e.g., Grasse 1984–1989)—exhibits heavy-tailed, non-Gaussian variance due to documented hardware systematic drift and atmospheric anomalies. The headline Nordtvedt extraction is therefore the precision-weighted full-systematic regression in The precision-weighted regression: every retained shot ($N = 25{,}445$) enters with inverse station-RMS weights and cluster-robust standard errors on the $\cos D$ coefficient, so high-variance hardware eras are down-weighted without deleting rows. Unweighted full-systematic OLS on the same design is reported as a sensitivity upper bound. When the focus shifts to residual correlation structure in the unweighted row, The combined estimator supplies cluster-robust standard errors combined with AR(1) pre-whitening. Robustness is further checked by a weighted biweight M-estimator, Student-t MCMC, Theil-Sen, and naïve cosD-only OLS.
...
#### 4.23.2 Robustness Checks and Diagnostic Tests
...
#### 4.23.4 Outlier-Threshold Sensitivity Sweep
...
#### 4.23.5 Prediction Interval Coverage and Uncertainty Calibration
...
The differential phase test recovers the same negative sign and an order-of-magnitude compatible amplitude, but its larger magnitude ($-5.95 \times 10^{-4}$ vs $-3.91 \times 10^{-4}$, ratio 1.52) reflects structural differences in the estimator. The differential uses a mean-difference between new-moon ($N = 393$) and full-moon ($N = 1{,}506$) bins — a 3.8:1 imbalance — while the primary regression fits a slope across all elongation phases. New-moon observations are taken in daytime (higher thermal noise), full-moon at night (lower noise), so the mean-difference is weighted toward the noisier daytime bin. A projection of the headline regression onto the differential bin structure gives an expected differential $\eta \approx -5.2 \times 10^{-4}$, within $1\sigma$ of the observed $-5.95 \times 10^{-4}$. The phase-locked test is therefore treated as a sign-and-phase diagnostic that cancels common-mode systematics, not as an independent precision estimate of $\eta$.

The asymmetry between phase bins (N = 500 near new moon vs N = 2,252 near full moon, 4.16:1 ratio) reflects a genuine observational constraint: LLR observations are geometrically challenging when the Moon is close to the Sun on the sky due to sky background and safety constraints. This asymmetry is a feature of the data, not an analysis artifact.
...
### 4.25 Station-by-Station Consistency
...
In practice, extracting an $\eta \sim 10^{-4}$ signal requires deep observation counts and sub-decimeter precision. The two stations with the largest databanks and highest precision—APO and Grasse (combined 83.9% of observations)—both yield negative $\eta$ from the regression fit. Grasse alone achieves conventional statistical significance ($\text{SNR} = 6.52\sigma$). APO falls short at $0.40\sigma$ (cosD-only). Three stations are underpowered: Matera ($N = 345$, expected SNR = $1.24\sigma$); McDonald2 ($N = 3,097$, severe phase truncation, mean $\cos D = -0.316$); and Haleakala (1984–1990, $N = 666$, 13.8 cm RMS, expected SNR = $0.81\sigma$). Haleakala's measured cosD-only $\eta = -4.75 \times 10^{-4}$ is consistent with noise at its $0.28\sigma$ precision level. Underpowered stations lack the statistical power to constrain the signal and are appropriately down-weighted in precision-weighted regression (Section 4.30.5) while retained for validation.

#### 4.31.1 Haleakala Audit
...
**Table 1: Station-level cosD-only regression estimates** — OLS fit of $\delta r = A \cos(D) + \epsilon$ per station on the cleaned sample, with $\eta = A / (13\,\text{m})$. Full-systematic per-station fits (not shown) yield different values: Grasse $\eta = -4.80 \times 10^{-4}$ ($6.81\sigma$), APO $\eta = -7.99 \times 10^{-5}$ ($0.78\sigma$), McDonald2 $\eta = -5.47 \times 10^{-4}$ ($1.21\sigma$), Matera $\eta = +5.99 \times 10^{-4}$ ($0.66\sigma$), Haleakala $\eta = -4.75 \times 10^{-4}$ ($0.16\sigma$). The Matera sign flip between cosD-only and full-systematic reflects the small sample ($N = 345$) and high variance; it is not statistically significant in either model.

| Station | $N$ (cleaned) | $\eta$ | $\sigma_\eta$ | $\eta/\sigma_\eta$ |
| --- | --- | --- | --- | --- |
| APO | 2,595 | $-7.99 \times 10^{-5}$ | $2.01 \times 10^{-4}$ | 0.40 |
| Grasse | 18,742 | $-4.80 \times 10^{-4}$ | $7.36 \times 10^{-5}$ | 6.52 |
| Matera | 345 | $+5.99 \times 10^{-4}$ | $9.63 \times 10^{-4}$ | 0.62 |
| McDonald2 | 3,097 | $-5.47 \times 10^{-4}$ | $3.44 \times 10^{-4}$ | 1.59 |
| Haleakala | 666 | $-4.75 \times 10^{-4}$ | $1.69 \times 10^{-3}$ | 0.28 |
...
### 4.30 Robustness and Systematic Error Analysis

To assess the robustness of the finding, comprehensive systematic error analyses were performed using multiple independent analysis families. Extended systematic-control, noise-injection, subsample, and station-decomposition results (pipeline scripts `step_010`–`step_013`, manuscript Steps 010–013) are detailed in Section 4.31. The key results are:
...
- 
Theil-Sen (cosD-only; the Cook leverage excision, consolidated in the canonical estimator hierarchy):
...
#### 4.30.2 Systematic Projection Analysis
...
#### 4.30.3 Phase-Locked Differential Analysis
...
### 4.31 Power and Sensitivity Analysis
...
### 4.8 Temporal Evolution and Synodic Phase Coherence
...
#### 4.25.1 Systematic Control Analysis
...
#### 4.25.2 Noise Injection and Signal Recovery
...
- 
Station-balanced IPW and precision-weighted regression (the station power analysis;
Section 4.30.5): A
...
underpowered sets like Haleakala ($N=666$) and severely
...
**Savage–Dickey prior and bandwidth sensitivity.** Table 4.8.1 documents four uniform $\eta$ priors with the intercept prior held fixed. Each row recomputes $\mathcal{B}_{\rm SD} = \pi(\eta{=}0)/p(\eta{=}0|{\rm data})$ under its own MCMC posterior (reference: 5,000 steps; sensitivity priors: 2,500 steps).

Table 4.8.1: Savage–Dickey sensitivity to $\eta$ prior and KDE bandwidth.
...
The headline quantitative claim remains the precision-weighted cluster-robust estimand at $6.94\sigma$. Stable Bayesian components are posterior exclusion of zero, Table 4.8.1, and the grid/bridge cross-checks; Laplace and single-kernel Savage–Dickey values are not discovery statistics.

### 4.30 Spectral Analysis and Frequency Specificity
...
### 4.31 Full Hard Audit Verification
...
### 4.14 Demonstration of Ephemeris Absorption Masking
...
#### 4.7.1 Orbital Velocity Modulation of Temporal Shear
...
Both terms remain significant after removing the distance component from $v_\parallel$. The orthogonalized model improves over the simple synodic model by $\Delta\text{AIC} = -132.6$, exceeding the original joint fit. This supports the interpretation that the $v_\parallel$ channel is not a mathematical alias of heliocentric distance once $r$ is removed.
...
The anti-CMB result exhibits dipole antisymmetry: reversing the axis reverses the sign while preserving the magnitude to within 0.1%, consistent with dipole geometry. The perpendicular directions are substantially weaker ($\approx 2.4\sigma$ vs $7.9\sigma$), though still marginally significant.
...
#### 4.7.3 Canonical Full-Systematic Extraction
...
#### 4.7.4 Per-Station Power and Observed Extraction
...
| --- | --- | --- | --- | --- | --- |
| Full pooled (cosD-only) | 25,445 | — | — | $-3.18 \times 10^{-4}$ | $5.25\sigma$ |
...
No single station reaches conventional standalone significance; the headline detection is carried by the pooled full-systematic model (Section 4.30.3). Grasse-C-SPAD hardware epochs reach $2.37\sigma$–$8.83\sigma$ depending on the partition, with negative $\eta$ in every powered epoch.

#### 4.7.5 Precision-Weighted Station Regression
...
at $6.27\sigma$ significance. This bounds the amplitude under quality-weighted pooling and complements the headline precision-weighted extraction (Section 4.30.3). The clean-subset Grasse C-SPAD-era plus APO analysis is reported separately in Section 4.31.

#### 4.7.6 Frequency Domain Orthogonality and Sideband Harmonics
...
### 4.17 Decoupling Thermal Array Deformation
...
**Per-Station cosD-Only Regression.** The validation analysis independently replicates the per-station cosD-only analysis in Table 1 (Section 4.24). The cosD-only OLS yields: Grasse $\eta = -4.80 \times 10^{-4}$ ($6.52\sigma$; per-station robustness check), APO $\eta = -7.99 \times 10^{-5}$ ($0.40\sigma$; per-station robustness check), McDonald2 $\eta = -5.47 \times 10^{-4}$ ($1.59\sigma$; per-station robustness check), Haleakala $\eta = -4.75 \times 10^{-4}$ ($0.28\sigma$, underpowered; Haleakala underpowered-station diagnostic), and Matera $\eta = +5.99 \times 10^{-4}$ ($0.62\sigma$, underpowered; per-station robustness check). Only Grasse reaches conventional significance in the cosD-only per-station model; the pooled full-systematic design is required to extract the common signal across all stations.
...
Primary inference therefore remains the precision-weighted pooled extraction (Section~3.4.22 and the precision-weighted regression): temporal CV is reported as a diagnostic on nuisance transport, not as a substitute estimand for $\eta$. The clean-subset analysis in Section~4.31 sharpens the same story under controlled homogeneity.

#### 4.33.1 Rolling-window $\eta(t)$ and environmental correlation
...
**Robustness check — Clean Subset: Grasse C-SPAD Era (2010+) + APO.** Grasse with the C-SPAD detector (2010 onwards) achieves ~2 cm RMS residuals and continuous coverage. APO provides a separate high-precision station with excellent phase coverage (mean $|\cos D| = 0.517$). Excluding Haleakala (opposite sign, underpowered), Matera (severe phase truncation, $N = 345$), and McDonald2 (moderate truncation, larger noise) yields a homogeneous subset of $N = 12\,576$ observations with residual RMS 2.5 cm.
...
### 4.32 Bounds on Solar Radiation Pressure and Atmospheric Seeing Systematics
...
#### 4.32.3 Atmospheric Seeing

Atmospheric seeing produces fast stochastic wander of the laser beam ($\sim$1 arcsec FWHM typical, 3 arcsec in poor conditions). Over LLR integration times, this wander averages to zero. The only plausible synodic-correlated channel is an elevation-dependent systematic: because new-moon observations occur at lower solar altitudes (daytime) and full-moon observations at higher altitudes (nighttime), an elevation-dependent seeing bias could alias into $\cos(D)$. This channel is already bounded by the the day/night thermal bias analysis day/night thermal bias null test ($p = 0.281$). the threshold sweep isolates the seeing-specific contribution and finds a maximum plausible synodic-correlated range bias of 0.111 mm, or 2.7% of the TEP signal amplitude. Fast stochastic wander averages to zero; the coherent elevation channel is already falsified.

**Conclusion.** Solar radiation pressure, both as a local mechanical displacement ($< 10^{-9}$ mm) and as an orbital scaling effect (no supported $1/r^2$ signature), is far too small or incorrectly scaled to explain the detected modulation. Atmospheric seeing is bounded at 0.111 mm ($\sim$3% of signal). The systematic error budget in Section 3.4.16 is updated to include both bounds. These quantitative exclusions, together with the existing dust/thermal (Section 4.26) and local terrestrial systematic (Section 5.9) tests, close the currently nominated conventional explanations for the synodic-phase-locked residual modulation without claiming that every conceivable unmodeled coupling has been eliminated.

### 4.35 Generic PPN Falsification Test

The preceding sections are framed within the Temporal Equivalence Principle. A complementary question is whether the same residual-channel signal requires modified gravity in the standard Parametrized Post-Newtonian (PPN) framework, independent of any TEP assumptions. In PPN, the Nordtvedt parameter is

\begin{equation}
\eta = 4\beta - \gamma - 3 - \frac{10}{3}\xi - \alpha_1 + \frac{2}{3}\alpha_2 - \frac{2}{3}\zeta_1 - \frac{1}{3}\zeta_2
\end{equation}

where $\beta$ (nonlinearity) and $\gamma$ (spatial curvature) are the Eddington parameters, both equal to unity in General Relativity. With the Cassini 2003 bound $|\gamma - 1| < 2.3 \times 10^{-5}$ (Bertotti et al. 2003) and the remaining PPN parameters ($\alpha_1, \alpha_2, \xi, \zeta_1, \zeta_2$) constrained to zero by Lunar Laser Ranging and other solar-system tests (Will 2014), the measured LLR Nordtvedt parameter provides a direct test of whether $\beta$ deviates from unity.

Using the headline precision-weighted estimand $\eta = -3.91 \times 10^{-4} \pm 5.63 \times 10^{-5}$ ($6.94\sigma$ deviation from GR; $p = 3.79 \times 10^{-12}$), and assuming the Cassini constraint on $\gamma$, the implied single-parameter PPN coupling is $\beta = 0.99990 \pm 0.00002$ ($6.43\sigma$ deviation from $\beta = 1$; $p = 1.29 \times 10^{-10}$).

Because the Cassini $\gamma$ and LLR $\eta$ constraints are statistically independent, a joint $(\beta, \gamma)$ confidence contour is constructed from the combined $\chi^2$:

\begin{equation}
\chi^2(\beta, \gamma) = \frac{(4\beta - \gamma - 3 - \eta)^2}{\sigma_\eta^2} + \frac{(\gamma - \gamma_{\rm Cassini})^2}{\sigma_\gamma^2}
\end{equation}

The exact minimum is at $\beta = 1 + \eta/4 = 0.99990$, $\gamma = \gamma_{\rm Cassini} = 1$, with $\chi^2_{\rm min} = 0$. The Hessian at the minimum yields the joint covariance matrix; projected one-dimensional errors are $\sigma_\beta = 1.07 \times 10^{-5}$ and $\sigma_\gamma = 1.63 \times 10^{-5}$. General Relativity ($\beta = \gamma = 1$) lies at $\Delta\chi^2 = 48.2$ relative to the best fit, placing it well outside the 99\% confidence contour ($\Delta\chi^2_{99\%, 2\,\mathrm{d.o.f.}} = 9.21$).

PPN parameter constraints from LLR and Cassini.

| Parameter | Value | Error | Deviation from GR |
| --- | --- | --- | --- |
| $\eta$ (LLR Nordtvedt) | $-3.91 \times 10^{-4}$ | $5.63 \times 10^{-5}$ | $6.94\sigma$ ($p = 3.79 \times 10^{-12}$) |
| $\gamma$ (Cassini) | $1.000000$ | $2.30 \times 10^{-5}$ | --- |
| $\beta$ (single constraint, $\gamma$ fixed) | $0.99990$ | $1.52 \times 10^{-5}$ | $6.43\sigma$ ($p = 1.29 \times 10^{-10}$) |
| $\beta$ (joint contour) | $0.99990$ | $1.07 \times 10^{-5}$ | $\Delta\chi^2 = 48.2$ vs GR; outside 99\% |
| $\gamma$ (joint contour) | $1.000000$ | $1.63 \times 10^{-5}$ | $\Delta\chi^2 = 48.2$ vs GR; outside 99\% |

The conclusion is independent of TEP: both $\eta \neq 0$ and $\beta \neq 1$ are detected at greater than $3\sigma$ within the standard PPN formalism. If $\beta$ deviates from unity, scalar-tensor theories are implicated regardless of whether the underlying mechanism is TEP or another framework. This test does not distinguish between scalar-tensor models; it shows that the LLR residual signal is inconsistent with General Relativity interpreted through the standard PPN lens.

### 4.36 Full Sky-Scan with Look-Elsewhere Correction

Section 4.7.2 reports a joint-model CMB orientation anisotropy ($\eta_\theta = -9.76 \times 10^{-4}$, $t = -11.03$) relative to the Planck dipole axis ($\mathrm{RA} = 168.14^\circ$, $\mathrm{Dec} = -7.22^\circ$). That analysis tests a single pre-specified direction. To guard against cherry-picking, the full residual channel is scanned over the celestial sphere in uniform 5° steps, yielding 2,664 independent sky directions. At each direction $\hat{\mathbf{n}}$, a directional modulation model $\delta r = a\cos D + c(\cos\theta_{\rm EM}\cdot\cos D) + b + \varepsilon$ is fit, where $\cos\theta_{\rm EM}$ is the cosine of the angle between the Earth-Moon line and $\hat{\mathbf{n}}$. Model comparison against the base synodic-only model uses the Akaike Information Criterion difference $\Delta\mathrm{AIC}$.

Across 2,664 directions, the maximum $\Delta\mathrm{AIC}$ is $106.48$ at $(\mathrm{RA} = 65.0^\circ, \mathrm{Dec} = -70.0^\circ)$. The Planck dipole direction attains $\Delta\mathrm{AIC} = 86.82$ and ranks 226 out of 2,664 (top 8.5\%). The median $\Delta\mathrm{AIC}$ across all directions is $33.6$; 90th, 95th, and 99th percentiles are $85.4$, $90.4$, and $99.7$ respectively, indicating that large $\Delta\mathrm{AIC}$ values are not rare on the real data but their spatial concentration matters.

To correct for the look-elsewhere effect inherent in scanning 2,664 directions, a scrambled-sky null distribution is generated by randomly permuting the $\cos\theta$ values across observations while preserving the marginal distribution and the synodic base model. This destroys any true directional correlation while maintaining the sampling geometry and residual structure. With $n = 1{,}000$ independent scrambled realizations, the null distribution of the maximum $\Delta\mathrm{AIC}$ has median $0.12$, 95th percentile $4.93$, and 99th percentile $8.31$.

Look-elsewhere-corrected p-values from the scrambled-sky null.

| Test | Observed $\Delta\mathrm{AIC}$ | Null 99th %ile | LEC $p$-value |
| --- | --- | --- | --- |
| Maximum over all 2,664 directions | $106.48$ | $8.31$ | $< 0.001$ ($n = 1{,}000$) |
| Planck dipole direction specifically | $86.82$ | --- | $< 0.001$ ($n = 1{,}000$) |

The look-elsewhere-corrected p-value for any axis exceeding the observed maximum is $p < 0.001$; for the Planck dipole specifically, the p-value is also $p < 0.001$. The directional signal therefore survives look-elsewhere correction. The Planck dipole is not the single highest-$\Delta\mathrm{AIC}$ direction (the maximum is at RA $= 65^\circ$, Dec $= -70^\circ$), but it lies in the top 8.5\% of all directions tested, and its specific null distribution is strongly excluded. These results are consistent with the joint-model directional structure reported in Section 4.7.2, but replace the step_055 uniform-axis Monte Carlo ($p_F \approx 0.096$) with a more rigorous scrambled-sky look-elsewhere correction.
...
The precision-weighted full-systematic headline fit to INPOP19a LLR residuals, after controlling for annual, monthly, and thermal $\cos(2D)$ aliases, yields $\eta = -3.91 \times 10^{-4} \pm 5.63 \times 10^{-5}$ ($6.94\sigma$; $N = 25{,}445$). Because only $G=5$ stations contribute, the co-primary uncertainty companion is a wild cluster bootstrap with Webb 6-point weights on the unweighted full-systematic OLS design, giving a $95\%$ percentile interval $[-5.37, -2.73] \times 10^{-4}$ ($5.56\sigma$; wild cluster bootstrap), supporting the inference under Webb weights. A station-era block bootstrap (10 blocks, 5{,}000 draws) yields only $1.40\sigma$ ($p = 0.14$); this weakness is expected because with only $\sim$10 highly unequal blocks the bootstrap standard error conflates structural between-block heterogeneity (phase truncation, noise levels, observational design) with sampling variation. The Webb wild cluster bootstrap remains the appropriate small-$G$ inference tool. The unweighted full-systematic OLS on the same design, reported as a sensitivity upper bound, gives $\eta = -4.06 \times 10^{-4} \pm 6.58 \times 10^{-5}$ ($6.17\sigma$; $6.52\sigma$ cluster-robust for comparison). Cook's-Distance excision on that unweighted row ($N = 23{,}837$) returns $\eta = -3.87 \times 10^{-4} \pm 4.95 \times 10^{-5}$ ($7.82\sigma$; $8.65\sigma$ cluster-robust) as a secondary leverage diagnostic. Numerical consolidation of the synodic spine, station pooling, and extended diagnostics appears in Section 4 (read order in Section 4.0).

Complementary to that headline estimand, a joint dynamical regression layer in the same residual channel tests whether additional structure survives after synodic, heliocentric, and CMB-orientation terms are fit together (the heliocentric radial velocity and CMB dipole orientation analyses). Orientation remains significant ($t = -11.03$) after heliocentric controls, improving the best purely heliocentric model by $\Delta\text{AIC} = -119.2$. The falsification suite excludes aliasing, multicollinearity, and permutation artifacts for the orientation term, while the joint-model directional anatomy battery passes. A full-sky scan with look-elsewhere correction (Section 4.36) places the Planck dipole direction at rank 226 out of 2,664 uniformly spaced directions (top 8.5\%), with a scrambled-sky null of $n = 1{,}000$ independent Monte Carlo realizations yielding a look-elsewhere-corrected $p < 0.001$ for both the any-axis maximum and the Planck dipole specifically. This supersedes the earlier step_055 uniform-axis Monte Carlo ($p_F \approx 0.096$) with a more rigorous null that preserves the marginal distribution and sampling geometry while destroying directional correlation. A composite specificity score combining aliasing, permutation, phase-null, orthogonalization, full-sky LEC, and directional-anatomy tests via Stouffer's Z method yields $Z = 2.14$ ($p = 0.016$), quantifying the combined evidence across independent test geometries. Correlation-matched scrambling is substantially less specific ($p_F \approx 0.266$). By contrast, a synodic phase null that circularly shifts the CMB $\cos\theta$ series while preserving its marginal distribution rejects chance alignment ($p_{F,{\rm eff}} = 6 \times 10^{-4}$ on 5,000 shifts; median scrambled $F_{\rm eff} \approx 1.6$ versus $F_{\rm eff}^{\rm CMB} \approx 50$). Gram–Schmidt orthogonalized direction scrambling remains marginal ($p_{F,{\rm eff}} \approx 0.090$), and a $30^\circ$ local rotation cone about the Planck axis is less specific still ($p_{F,{\rm eff}} \approx 0.30$). The G2 falsification test shows galactic north attains slightly larger $\Delta\mathrm{AIC}$ in single-axis fits, but the dual-axis test (H) finds $r(\cos\theta_{\rm CMB}, \cos\theta_{\rm gal}) = 0.984$: when both are fit together, only the galactic coefficient remains marginally significant ($t = -2.63$) and the Planck term is absorbed ($t = 0.68$; add-CMB$|$gal $p = 0.50$). The orientation signal is therefore one shared subspace, not two independent axes. Anti-dipole antisymmetry and joint-model direction rotations are consistent with dipole geometry about the chosen operational axis. These joint-model results are strong corroborative dynamical evidence for fixed-sky directional structure on the same residual channel; they do not replace or numerically supersede the primary pooled $\eta$ estimand.
...
The finding is cross-checked across the estimator hierarchy in Section 4.21 and the independent spine in Section 4.0. Powered hardware-era splits and the headline precision-weighted estimand show consistent negative $\eta$ values, with no sign reversals among the tests that retain adequate phase coverage and $N$.
...
Precision-weighted full-systematic regression ($\eta = -3.91 \times 10^{-4} \pm 5.63 \times 10^{-5}$, $6.94\sigma$; the precision-weighted regression) is the headline cross-station estimator: every observation is retained while low-precision hardware is objectively down-weighted. The cosD-only precision-weighted diagnostic ($\eta_{\rm WLS} = -3.21 \times 10^{-4} \pm 5.12 \times 10^{-5}$, $6.27\sigma$; the station power analysis) shows the same sign under a simplified nuisance structure. Neither construction removes all Grasse leverage by construction; together they show that the negative phase-locked estimate is not solely an artifact of naive unweighted pooling. Outlier sweeps, resampling, cross-validation, and the systematic-error budget (Section 4.33) are documented in Results and are not repeated here.

### 5.2 Consistency with Prior TEP Constraints

The headline measurement $\eta = -3.91 \times 10^{-4} \pm 5.63 \times 10^{-5}$ ($6.94\sigma$), supported by a wild cluster bootstrap $95\%$ interval $[-5.37, -2.73] \times 10^{-4}$ ($5.56\sigma$), with Cook's leverage diagnostics and unweighted OLS bounds consistent in sign and magnitude, sharpens the previously ambiguous suppression phenomenology without closing it at the level of a first-principles prediction. Screening of Temporal Shear operates via the continuous spatial profile of the time field (Temporal Topology), in which Earth's substantially deeper compactness ($\sim 7 \times 10^{-10}$) relative to the Moon ($\sim 3 \times 10^{-11}$) produces stronger suppression of Temporal Shear, yielding the observed differential coupling.

Quantitative derivation utilizes the TEP geometric Temporal Shear formalism and the understanding that different astrophysical regimes exhibit distinct Observable Response Coefficients. The Nordtvedt parameter $\eta$ itself serves as the observable response coefficient for LLR, analogous to preliminary $\kappa_{\rm Cep} \sim 10^6$ mag for Cepheids and $\kappa_{\rm MSP} \sim 10^6$–$10^7$ for pulsars from related work in the same framework (Papers 10 and 11). The measured $\eta \approx -3.9 \times 10^{-4}$ is substantially smaller than these galactic-scale coefficients, consistent with the screening mechanism: LLR operates in a more screened Solar System regime where Temporal Shear suppression is stronger, yielding a smaller effective response. The geometric Temporal Shear formalism motivates an order-of-magnitude phenomenological range $\eta \sim -10^{-4}$, consistent with the headline pooled parameter near $-3.9 \times 10^{-4}$.
...
The residual-channel signal of a non-zero $\eta$ has significant implications for gravity theories. In General Relativity (GR), $\eta = 0$ exactly. The residual data extract a statistically robust synodic modulation at $6.94\sigma$ from the precision-weighted full-systematic INPOP19a headline fit, with a wild cluster bootstrap $95\%$ interval $[-5.37, -2.73] \times 10^{-4}$ ($5.56\sigma$) confirming the detection is not an artifact of unreliable small-$G$ asymptotics. Cook's-Distance leverage diagnostics reach $7.82\sigma$ ($8.65\sigma$ cluster-robust) on the unweighted excised row. This is strong residual-channel evidence consistent with TEP because the effect has the predicted synodic functional form, the predicted negative sign, and a stable amplitude across the canonical estimator hierarchy. The modeled known-systematic-only explanations fail: every known systematic injected at its observed amplitude fails to reproduce the observed $|\eta|$ (the systematic sensitivity analysis; 2,000 deterministic simulations per systematic, all fractions $0.0\%$). Adversarial PCA on an 82-term basis leaves $\eta = -2.93 \times 10^{-4}$ ($2.95\sigma$) with 20 residual-ranked components; blind 20% year hold-out yields $\eta = -3.66 \times 10^{-4}$ ($14.3\sigma$) with nuisances fit only on training years. A 2D elongation$\times$time GP partially overlaps $\cos(D)$ on the sampling manifold ($\approx 83\%$ amplitude reduction after subtraction), but cross-validated GP generalisation tests show this absorption is overfitting: the CV-GP overfit ratio is 1.42 and the cosD coefficient on CV-GP residuals is $\eta = +2.45 \times 10^{-5}$ ($0.42\sigma$), while a time-only GP leaves cosD intact at $5.50\sigma$, confirming the elongation dimension is where the GP overfits. Era$\times$station$\times$lunation cells ($N \geq 80$) show sign consistency in 13/26 fitted partitions (multiplicity control treats these as sensitivity tests). A simple Grasse-specific differential $\cos(D)$ systematic is disfavored by interaction and partition tests, and equal-weighted as well as balanced-station-N reweightings preserve the negative sign (though both are underpowered by construction), while retaining bounded station-leverage risk. TEP provides a concrete scalar-tensor framework in which this residual-channel pattern has a natural interpretation.

The residual-channel evidence is more naturally aligned with TEP than with a strictly static GR residual model for the following reasons: (1) The residual analysis extracts a statistically robust synodic modulation at $6.94\sigma$ from the precision-weighted full-systematic INPOP19a headline model, with a wild cluster bootstrap $95\%$ interval $[-5.37, -2.73] \times 10^{-4}$ ($5.56\sigma$) confirming the detection survives correction for small-$G$ unreliability, and with consistent Cook's-leverage diagnostics at higher SNR on an excised row; (2) The signal exhibits the precise functional form expected under the TEP framework ($\cos(D)$ modulation) with the correct sign; (3) The heliocentric gradient dependence (perihelion enhancement) is compatible with the qualitative form of TEP's scalar field prediction and difficult to reconcile with a purely static GR residual model. GR requires $\eta = 0$ in a completed direct-fit model; the residual data indicate a non-zero post-fit Nordtvedt-like coefficient consistent with TEP's modified gravity framework. A full dynamical ephemeris refit incorporating TEP's scalar field dynamics is the natural next external confirmation.

The theory is designed to remain compatible with other precision tests through TEP geometric suppression: Solar System tests (Cassini; Bertotti et al. 2003), lunar laser ranging, and related constraints are addressed by suppression in dense environments; cosmological constraints (BBN, CMB, $\sigma_8$) are addressed through Yukawa suppression on large scales; and the theory makes testable predictions for other systems (wide binaries, globular cluster pulsars, etc.). Those compatibility arguments remain part of the broader TEP program rather than direct consequences of the residual-channel fit alone.
...
is 1.35 mm, compared to the observed 4.24 mm signal—a 3.1× mismatch.
...
4.16 years) is addressed by restricting INPOP19a to the DE430
...
modulation expected under the TEP framework: the signal strength scales
...
scalar-field gradient — a TEP expectation.
...
Four additional robustness checks corroborate dipole-like structure about the operational Planck axis while exposing limits on uniqueness. The directional specificity test exhibits dipole antisymmetry: the true anti-CMB antipode yields $\Delta\eta = +1.76 \times 10^{-3}$ ($7.92\sigma$; CMB-directional-specificity robustness check), a sign-reversed response with $|\Delta\eta_{\rm anti}| / |\Delta\eta_{\rm CMB}| = 1.000$. The two rigorously perpendicular directions are suppressed to $0.35\times$ and $0.26\times$ the CMB amplitude ($\approx 2.4\sigma$ vs $7.9\sigma$), though still marginally significant.
...
*Directional specificity in the joint model.* A full-sky scan
with look-elsewhere correction (Section 4.36; 2,664 directions,
$n = 1{,}000$ scrambled-sky Monte Carlo) yields $p < 0.001$ for both
the any-axis maximum and the Planck dipole specifically, superseding
the earlier uniform-axis strand ($p_F \approx 0.096$).
A composite specificity score across aliasing, permutation, phase-null,
orthogonalization, full-sky LEC, and directional-anatomy tests gives
$Z = 2.14$ ($p = 0.016$). Independent $90^\circ$ rotations and the
$180^\circ$ antipode rule out
arbitrary directional alignment relative to that axis.
Correlation-matched scrambling gives $p_F \approx 0.266$. Refined nulls
in the same step separate strands of the directional claim: synodic phase
scrambling is rejected ($p_{F,{\rm eff}} = 6 \times 10^{-4}$),
whereas uniform-direction, Gram–Schmidt-orthogonalized, and local
($30^\circ$) axis-rotation nulls remain marginal to conservative
($p_{F,{\rm eff}} \approx 0.09$–$0.30$). The dual-axis fit shows
galactic north's single-axis $\Delta\mathrm{AIC}$ advantage is not a separable
second axis: $r(\cos\theta_{\rm CMB},\cos\theta_{\rm gal}) = 0.984$, and the
Planck coefficient is absorbed when both are included ($t = 0.68$; add-CMB$|$gal
$p = 0.50$). Combined with the aliasing,
permutation, and orthogonalization tests above, the Planck-aligned
structure is consistent with a synodically phased dipole component of
the joint dynamical model; uniform sky-scrambling is reported only as a
conservative bound on unique axis identity, not as a strict-threshold pass.
...
*Scope note:* Sections 5.10–5.22 are programmatic predictions and interpretative extensions of the TEP framework. They do not add independent estimands to the pooled full-systematic synodic signal in Section 4.34; instead, they define nearby lunar and metrological observables on which the same time-field architecture can be tested. Quantitative modeling of these effects remains an open analytical target.
...
A long-standing puzzle in Earth-Moon evolution is that present-day tidal dissipation rates, if extrapolated linearly to the past, imply a lunar age of ~1.5 Gyr rather than the accepted ~4.16 Gyr (*Geology* 2016; Hansen 1982). The standard resolution—that tidal dissipation was significantly weaker in Earth's past—lacks a physical mechanism explaining why or how this would occur.
...
Conditioned on the primary synodic fit, joint regressions show corroborative fixed-sky directional structure when the Planck dipole axis is used as the operational predictor ($\eta_\theta = -9.76 \times 10^{-4}$, $t = -11.03$), while heliocentric distance remains non-significant ($p = 0.171$). A full-sky scan with look-elsewhere correction (Section 4.36) places the Planck dipole at rank 226 out of 2,664 directions (top 8.5\%) with a scrambled-sky null of $n = 1{,}000$ yielding $p < 0.001$ for both the any-axis maximum and the Planck dipole specifically. Gram–Schmidt direction nulls remain marginal ($p_{F,{\rm eff}} \approx 0.09$), while a composite specificity score across aliasing, permutation, phase-null, orthogonalization, full-sky LEC, and directional-anatomy tests yields $Z = 2.14$ ($p = 0.016$). Galactic north shares a nearly collinear subspace ($r = 0.984$) rather than resolving a second independent axis. Section 4.33.2 reports the full dipole anatomy; this layer reinforces the scalar-field architecture but does not replace the synodic $\eta$ estimand.
...
Independent of the TEP framework, the measured Nordtvedt parameter implies $\beta = 0.99990 \pm 1.07 \times 10^{-5}$ from the joint $(\beta, \gamma)$ confidence contour within the standard Parametrized Post-Newtonian formalism (Section 4.35). General Relativity ($\beta = \gamma = 1$) lies at $\Delta\chi^2 = 48.2$, well outside the 99\% contour. This indicates that the LLR residual signal is inconsistent with General Relativity in the standard PPN lens, suggesting modified gravity regardless of whether the underlying mechanism is TEP or another scalar-tensor framework.

These results provide substantial residual-channel evidence consistent with the TEP hypothesis of a dynamical proper time field $\phi$, in the specific sense of a nonzero synodic Nordtvedt-like coefficient in post-fit LLR residuals. Within the TEP framework, the same synodic footprint supplies a candidate physical anchor for the Müller & Nordtvedt (1998) post-model residual and for the adjacent LLR anomaly threads discussed in Section 5. Broader claims about light propagation and coordinated lunar-time standards remain interpretive extensions of the framework, not conclusions forced by the residual-channel fit alone.

### 6.1 Synthesis of Results

Section 4.0 and the read-order guide in Section 4 consolidate the synodic spine before station critiques, multiplicity control, and extended diagnostics (especially the robustness, cross-validation, and station-balance sections). Universal sign and pooling are assessed jointly through the common-$\eta$ mixed model ($F = 1.19$, $p = 0.31$), not through equal per-station $\sigma$ thresholds.

DE430 (short baseline, outlier sensitivity) is an independent cross-ephemeris strand. The Planck-axis joint layer is corroborative fixed-sky structure whose full-sky look-elsewhere-corrected specificity is $p < 0.001$ ($n = 1{,}000$ scrambled-sky Monte Carlo; Section 4.36) and whose axis is not uniquely identified against galactic north (Section 4.33.2). Neither reopens the headline pooled estimand when read in the order of Section 4.

### 6.2 Interpretative Framework

The extracted residual-channel coefficient, $\eta \approx -4 \times 10^{-4}$ across the estimator hierarchy, sharpens the suppression phenomenology in TEP. The negative sign supports the interpretation that gravitational potential scaling (Temporal Shear suppression $\propto \Phi/c^2$) dominates over soliton-radius scaling in the Earth-Moon system.
...
While TEP does not make a precise quantitative prediction for $\eta$ from a single universal coupling constant due to suppression model uncertainties and domain-specific astrophysical amplification, the observed Nordtvedt variance is within the order-of-magnitude parameter range expected from the formal TEP framework (centered on the $10^{-4}$ regime in the phenomenological volumetric model) and provides qualitative geometric parity across stellar dynamics, high-z galaxies, and planetary ephemerides.
...
The analysis pipeline comprises 82 sequential steps organized into logical groups. Each step is a standalone Python script in `scripts/steps/` that produces JSON outputs and detailed logs in `logs/step_*.log`. Manuscript step labels in Results and Discussion follow the `step_XXX` script prefix wherever possible (e.g. the primary extraction $\leftrightarrow$ `step_050_corrected_tep_analysis.py`). Where a script's internal docstring uses a different ordinal, the manuscript cites the script prefix and the Results subsection given in the mapping table below.

#### Complete Step Inventory & Runtime

The canonical pipeline comprises 82 sequential steps organized into logical groups. The core statistical analysis (steps 000–003) provides the primary signal extraction, while steps 004–076 perform extended systematic checks, robustness validation, and theoretical consistency tests. Recent enhancements include:
...
| --- | --- | --- | --- |
| 010–013 | `step_010`–`step_013` | `step_010`–`step_013` outputs | §4.31.1–4.31.4 |
| 029 (day/night) | `step_027_day_night_thermal_bias.py` | `step_027_day_night_thermal_bias.json` | §4.25.1 |
| 029 (station power / WLS) | `step_029_station_power_analysis.py` | `step_029_station_power_analysis.json` | §4.30.4–4.30.5 |
| 030 (geometric elongation) | `step_028_geometric_elongation.py` | `step_028_geometric_elongation.json` | §4.25.2 |
| 032 (hardware epochs) | `step_030_hardware_epoch_analysis.py` | `step_030_hardware_epoch_analysis.json` | §4.6 |
| 034 (orthogonality) | `step_032_ephemeris_orthogonality_proof.py` | `step_032_ephemeris_orthogonality_proof.json` | §4.30.6 |
| 045 (matched-window) | `step_045_independent_validation.py` | `step_045_independent_validation.json` | §4.0 / §5.4 |
| 050 (primary extraction) | `step_050_corrected_tep_analysis.py` | `step_050_corrected_tep_analysis.json` | §4.30.3 |
| 055 (CMB robustness suite; random-axis ΔAIC null; dual-axis primary; η_θ coverage) | `step_055_cmb_rigorous_falsification.py` | `step_055_cmb_rigorous_falsification.json` | §4.30.2 / §5.4 |
| 056 (linearized post-fit $\eta$ extraction) | `step_056_dynamical_integrator_eta_refit.py` | `step_056_dynamical_integrator_eta_refit.json` | §4.31 |
| Step 064-PI (prediction-interval calibration) | `step_064_prediction_coverage.py` | `step_064_prediction_coverage.json` | §4.20.5 |
| Step 072 (leave-one-station-out meta; forest plot) | `step_072_leave_one_station_out_meta.py` | `step_072_leave_one_station_out_meta.json` | §4.24 |
| Step 064-SRP (orbital SRP scaling) | `step_064_srp_systematic_check.py` | `step_064_srp_systematic_check.json` | §4.32.2 |
| 075 (generic PPN falsification) | `step_075_generic_ppn_test.py` | `step_075_generic_ppn_test.json` | §4.35 |
| 076 (full sky-scan with LEC) | `step_076_sky_scan_directional.py` | `step_076_sky_scan_directional.json` | §4.36 |
...
| Extended Systematic Analysis | 78 (004–076, including 006b and 046b) | ~6m 27s |
| Total | 82 | ~6m 53s |
...
- All 82 canonical steps complete with "PASS" status in pipeline output

- JSON files in `results/outputs/` (step_000 through step_076, including 006b and 046b)
...
- Key result (PPN falsification): Joint $(\beta, \gamma)$ contour from LLR $\eta$ and Cassini $\gamma$ places GR ($\beta = \gamma = 1$) at $\Delta\chi^2 = 48.2$, outside the 99\% contour; $\beta = 0.99990 \pm 1.07 \times 10^{-5}$ (Section 4.35)

- Key result (sky-scan LEC): Full-sky scan (2,664 directions, 5° grid) maximum $\Delta\mathrm{AIC} = 106.48$; Planck dipole rank 226/2664 (top 8.5\%); scrambled-sky null ($n = 1{,}000$) yields LEC $p < 0.001$ for any axis and Planck dipole specifically (Section 4.36)